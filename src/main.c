...
int handle_echo(struct client_info* c_info, char *echo_str, struct request_data *req_data);
int handle_user_agent(struct client_info* c_info, struct request_data *req_data);
...
void *handle_request_thread(void *c){
  ...
  while (1){
    ...
    int bytes_received;
    while ( (bytes_received = recv(client_fd, client_req, MAX_LENGTH, 0)) <= 0){
    ...
    for (int i = 0; i < req_data->header_data->length; i++){
      if (strcmp(headers[i]->name, "Connection") == 0 && strcmp(headers[i]->value, "close") == 0){
        c_info->close_connection = 1;
        break;
      }
    }

    if (c_info->close_connection){
    ...
    if (strcmp(req_data->request_path, "/") == 0){
      ...
      handle_echo(c_info, str, req_data);
    }
    else if (strncmp(req_data->request_path, "/user-agent", 11) == 0){
      handle_user_agent(c_info, req_data);
    }
...
int handle_file_request(void *c, struct request_data* req){
  struct client_info *c_info = (struct client_info *)c;
  long long int client_fd = c_info->client_fd;
...
int handle_echo(struct client_info* c_info,char *str, struct request_data* req_data){
  size_t headers_len = req_data->header_data->length;
  long long client_fd = c_info->client_fd;
  struct header **headers = req_data->header_data->headers;

  char *client_encodings = NULL;
